...
export default withRouter(
  class extends React.PureComponent {
    ...
    async loadData() {
      let poker = await action.post("PKGame/GoldenPoker", { type: 0 })
      if (poker.Code != 1) {
        apiNotification.alert(poker, {}, this.props)
        return
      }

      this.setState({ poker: poker.Data, loading: false })
    }
    ...
    openTable() {
      ...
      action.post("PKGame/Golden_Make", query, (res) => {
        this.setState({ apiLoading: false })
        if (res.Code == 1) {
          notificationAsync.alert(res.Message, { title: "成功" }).then(() => {
            this.props.router.isLoginToOrRedirect("/goldenPK/home")
          })
    ...
    async changeCard() {
      let poker = await action.post("PKGame/GoldenPoker", { type: 1 })
      apiNotification.alert(poker, {}, this.props)
      if (poker.Code == 1) {
        this.setState({ poker: poker.Data, changeCard: true })
      }
      return
    }
    ...
    render() {
      ...
      return (
        <LayoutPage
          ...
          renderFixed={() => <GoldenPKNavigatorBar active="create" PKGame="goldenPK" />}
        >
