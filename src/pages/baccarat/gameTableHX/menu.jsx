...
export default class extends React.PureComponent {
  constructor(props) {
    ...
    this.tableBetRecord = {
      tabs: [
        {
          name: "本桌注单",
          filter: [{ key: "lotteryid", type: "hidden", defaultValue: this.props.gameTableID }],
          listApi: "Lottery/Bets",
          listApiMethod: "post",
          renderRow: (row, data) => {
            let status = "未开奖"
            if (row.Status == 1) {
              status = "中奖" + row.Bonus + "元"
            ...
            return (
              <div className="recordItem" key={"betRecord" + row.ID}>
                <p className="tl">
                  时间：{util.date.format(util.date.toDate(row.BetTime), "YYYY-MM-DD hh:mm:ss")}
                  <span className="right">{row.Status == 1 ? <b style={{ color: "#c30202" }}>{status}</b> : status}</span>
                </p>
                <p className="dd">
                  <span>期数：{row.GameID}</span>
                  <span>玩家：{row.NickName}</span>
                  <span>类型：{row.PlayType}</span>
                  <span>金额：{row.BetCount} 元</span>
                  {/* <span>注单：{row.BetText} </span> */}
                </p>
              </div>
            )
          },
  ...
  onMenuClick(index) {
    if (index == 0) {
      ...
      action.post("Baccarat/GetHXLuZhi", { PageIndex: 1, PageSize: 15, ID: this.props.gameTableID }, (res) => {
        if (res.Code == 1) {
  ...
  renderRoundDetail() {
    ...
    return (
      <div className="round-info">
          ...
          <div className="poker dianItem">
            {pokers.Zhuan && pokers.Zhuan.length
              ? pokers.Zhuan.map((item, index) => {
                  return <Poker key={index} className={`zhuang poker-${index}`} value={item.code} shape={"hearts"} />
                })
              : null}
          </div>
          ...
          <div className="poker dianItem">
            {pokers.Xian && pokers.Xian.length
              ? pokers.Xian.map((item, index) => {
                  return <Poker key={index} className={`xian poker-${index}`} value={item.code} shape={"spades"} />
                })
              : null}
          </div>
        ...
        <div className="item hash-item">
          哈希验证：
          {roundDetail.HashCode && (
            <a className="hash-link" href={"https://tronscan.org/#/transaction/" + roundDetail.HashCode} target="_blank">
              ...{roundDetail.HashCode.substr(-16, 16)}
            </a>
          )}
        </div>
      </div>
  ...
  renderWinStatistic() {
    ...
    data.map((record) => {
      ...
      if (poker.Zdian > poker.Xdian) {
        ...
        // 开和
        if (lastWin == "Zdian" || lastWin == "Xdian") {
          column++
          statistic[column] = []
        }
        lastWin = "Draw"
    ...
    return (
      <div className="baccarat-menu">
        ...
        <ModalPage
          ...
          animation="lift"
          onClose={() => {
            this.setState({ showTableBetRecord: false })
          }}
        >
          <div>
        ...
        <ModalPage
          ...
          animation="lift"
          onClose={() => {
            this.setState({ showRoundRecord: false })
          }}
        >
          <div>
        ...
        <ModalPage
          ...
          className="report-record-modal round-detail-modal hx-baccarat record-status-modal"
          animation="fade"
          onClose={() => {
            this.setState({
              showRoundDetail: false,
        ...
        >
          {this.state.showRoundDetail && (
            <div className="modal-inner">
              {roundDetail && <div className="main-title">第 {roundDetail.GameID} 局</div>}
        ...
        {/*/!* 路纸 *!/*/}
        <Modal isOpen={this.state.showWinStatistic} className="report-record-modal win-statistic-modal hx-baccarat" animation="fade">
